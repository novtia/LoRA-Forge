...
interface SessionStore {
  ...
  createNew: (projectId: string) => Promise<string>;
...
export const useSession = create<SessionStore>((set, get) => {
  ...
  ensureActive: async () => {
    ...
    if (cur) return cur;
    const projectId = useProject.getState().projects[0]?.id;
    if (!projectId) {
      throw new Error("请先在主窗口创建 LoRA 训练项目");
    }
    return await get().createNew(projectId);
  },
