...
export const api = {
  ...
  createSession: (projectId: string, title?: string, model?: string) =>
    invoke<Session>("create_session", { args: { projectId, title, model } }),
  ...
  // projects
  listProjects: () => invoke<Project[]>("agent_list_projects"),
  createProject: (name: string, path?: string | null) =>
    invoke<Project>("agent_create_project", { args: { name, path: path ?? null } }),
