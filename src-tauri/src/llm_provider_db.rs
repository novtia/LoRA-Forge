...
use crate::{
    ...
    models::{
        resolve_effective_llm_settings, LlmGlobalSettings, LlmModelSource, LlmProvider,
        LlmProviderModelEntry, LlmProviderSummary, LlmSettings,
    },
...
struct LlmProviderConfigJson {
    ...
    endpoint_url: String,
    api_key: String,
    endpoint_kind: crate::models::EndpointKind,
...
fn insert_llm_provider(connection: &Connection, provider: &LlmProvider) -> AppResult<()> {
    ...
    connection.execute(
        "
        INSERT INTO llm_providers (id, config_json, sort_order, created_at, updated_at)
        VALUES (?1, ?2, ?3, ?4, ?5)
        ",
        params![
            provider.id,
...
pub fn list_llm_providers(connection: &Connection) -> AppResult<Vec<LlmProviderSummary>> {
    ...
    let mut out = Vec::new();
    for row in rows {
        let (id, json, created_at, updated_at) = row?;
        let provider = provider_from_row(&id, &json, created_at, updated_at)?;
...
pub fn get_llm_provider(connection: &Connection, provider_id: &str) -> AppResult<LlmProvider> {
    ...
    let (id, json, created_at, updated_at) = row;
    provider_from_row(&id, &json, created_at, updated_at)
}
...
pub fn load_llm_global_settings(connection: &Connection) -> AppResult<LlmGlobalSettings> {
    ...
    match maybe_json {
        Some(config_json) => {
            if config_json.contains("activeProviderId") {
                return Ok(serde_json::from_str(&config_json)?);
            }
            let legacy: LlmSettings = serde_json::from_str(&config_json)?;
            let mut global = LlmGlobalSettings::default();
            global.ingest_behavior_from(&legacy);
            Ok(global)
        }
...
pub fn resolve_effective_llm_settings_from_db(
    connection: &Connection,
    ...
    Ok(resolve_effective_llm_settings(
        &global,
        provider.as_ref(),
    ))
}
