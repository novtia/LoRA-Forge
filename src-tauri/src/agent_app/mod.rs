...
pub struct AgentAppState {
    ...
    /// Agent subsystem services. Kept on `AgentAppState` so that any Tauri
...
impl AgentAppState {
    ...
    /** ?? lora ????? agent ????? */
    pub fn merged_settings(&self) -> AppResult<settings::Settings> {
        self.lora
            .with_db(|lora_conn| {
                let agent_conn = self
                    .conn()
                    .map_err(|error| crate::error::AppError::State(error.to_string()))?;
                lora_provider_bridge::read_merged(&agent_conn, lora_conn)
                    .map_err(|error| crate::error::AppError::State(error.to_string()))
            })
    ...
    /** ? lora ???????????? */
    pub fn merged_projects(&self) -> AppResult<Vec<project::Project>> {
        self.lora
            .with_db(|lora_conn| {
                let agent_conn = self
                    .conn()
                    .map_err(|error| crate::error::AppError::State(error.to_string()))?;
                lora_project_bridge::list_projects(lora_conn, &agent_conn)
                    .map_err(|error| crate::error::AppError::State(error.to_string()))
            })
            .map_err(|error| AppError::Other(error.to_string()))
    }
...
fn clear_generation_abort(state: &AgentAppState, session_id: &str) {
...
#[allow(dead_code)]
pub struct EffectiveSessionParams {
...
pub struct SettingsChatFactory {
    pool: DbPool,
    lora: AppState,
...
impl SettingsChatFactory {
    fn new(pool: DbPool, lora: AppState, user_context: Arc<FsUserContextLoader>) -> Self {
        Self {
            pool,
            lora,
            user_context,
...
impl ChatRequestFactory for SettingsChatFactory {
    fn build(
        ...
        let settings = self
            .lora
            .with_db(|lora_conn| {
                lora_provider_bridge::read_merged(&conn, lora_conn)
                    .map_err(|error| crate::error::AppError::State(error.to_string()))
            })
            .map_err(|error| AppError::Other(error.to_string()))?;
...
pub fn update_settings(
        ...
        (patch.active_provider_id.as_deref(), patch.model.as_deref())
    {
        state
            .lora
            .with_db(|lora_conn| {
                lora_provider_bridge::set_active_selection(lora_conn, provider_id, model)
                    .map_err(|error| crate::error::AppError::State(error.to_string()))
            })
            .map_err(|error| AppError::Other(error.to_string()))?;
    }
    let conn = state.conn()?;
    settings::apply_agent_local_patch(&conn, patch)?;
    state.merged_settings()
}
...
pub fn list_sessions(
    ...
    let _ = state.merged_projects()?;
    let conn = state.conn()?;
    Ok(session::list(&conn)?
        .into_iter()
...
pub fn search_sessions(
    ...
    let _ = state.merged_projects()?;
    let conn = state.conn()?;
    Ok(session::search(&conn, &query, limit)?
        .into_iter()
        .filter(|item| item.project_id.is_some())
        .collect())
}
...
pub fn create_session(
    ...
    if project_id.is_empty() {
        return Err(AppError::Invalid(
            "project_id is required; create sessions inside a lora training project".into(),
        ));
    }
    let projects = state.merged_projects()?;
    if !projects.iter().any(|item| item.id == project_id) {
        return Err(AppError::Invalid(format!(
            "unknown lora project id: {project_id}"
        )));
    }

    let conn = state.conn()?;
    let mut sess = session::create(&conn, args.title, args.model)?;
    project::assign_session(&conn, &sess.id, Some(project_id))?;
    ...
    if sess.model.is_none() {
        if let Ok(s) = state.merged_settings() {
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateSessionConfigArgs {
    id: String,
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SetSessionModelArgs {
    id: String,
    model: String,
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SetSessionAgentTypeArgs {
    id: String,
    agent_type: String,
}
...
#[tauri::command]
pub fn agent_list_projects(
    state: tauri::State<Arc<AgentAppState>>,
) -> Result<Vec<project::Project>, AppError> {
    state.merged_projects()
}

#[tauri::command]
pub fn agent_create_project(
    _state: tauri::State<Arc<AgentAppState>>,
    _args: CreateProjectArgs,
) -> Result<project::Project, AppError> {
    Err(AppError::Invalid(
        "projects are managed by lora-forge; create them in the main app".into(),
    ))
}
...
pub fn rename_project(
    ...
    Err(AppError::Invalid(
        "project names are synced from lora-forge; rename them in the main app".into(),
    ))
}
...
pub fn delete_project(
    ...
    Err(AppError::Invalid(
        "projects are managed by lora-forge; delete them in the main app".into(),
    ))
}
...
pub fn assign_session_to_project(
    ...
    let pid = project_id
        ...
        .ok_or_else(|| {
            AppError::Invalid("sessions must belong to a lora training project".into())
        })?;
    let projects = state.merged_projects()?;
    if !projects.iter().any(|item| item.id == pid) {
        return Err(AppError::Invalid(format!("unknown lora project id: {pid}")));
    }
    let conn = state.conn()?;
    project::assign_session(&conn, &session_id, Some(pid))
}
...
pub async fn quote_message_as_attachments(
    state: tauri::State<'_, Arc<AgentAppState>>,
    app: AppHandle,
    session_id: String,
    message_id: String,
...
#[tauri::command]
pub fn list_agent_tasks(state: tauri::State<Arc<AgentAppState>>) -> Result<Vec<Task>, AppError> {
...
#[tauri::command]
pub fn list_agent_tools(state: tauri::State<Arc<AgentAppState>>) -> Result<Vec<String>, AppError> {
...
#[tauri::command]
pub fn list_agents(state: tauri::State<Arc<AgentAppState>>) -> Result<Vec<AgentSummary>, AppError> {
...
#[derive(Debug, Serialize)]
pub struct SessionWithMessagesAbs {
 ...
 */
pub fn initialize(app: &AppHandle, lora: AppState) -> AppResult<Arc<AgentAppState>> {
    let db_path = paths::db_path(app)?;
